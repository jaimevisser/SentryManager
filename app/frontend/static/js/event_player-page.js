...
export function initEventPlayer() {
    ...
    function syncRenderUI() {
        if (renderActionButton) {
            renderActionButton.disabled = renderInFlight || !playerRenderUrl || !(trimEndTime > trimStartTime);
        ...
        try {
            const response = await fetch(playerRenderUrl, {
                ...
                body: JSON.stringify({
                    ...
                    playerEdits: buildPlayerEditsPayload(),
                }),
            });
            const responsePayload = await response.json().catch(() => null);
            if (!response.ok) {
    ...
    function applySnapshotStageConfig() {
        ...
        if (width && height) {
            ...
            viewFrame.style.gap = activeLayout === "triple" ? "0px" : `${doubleGap}px`;
        } else {
    ...
    async function seekSnapshotFrame({ layout, cameraKey, segmentKey, clipTimeSeconds }) {
        const normalizedLayout = typeof layout === "string" ? layout : activeLayout;
        const normalizedCameraKey = typeof cameraKey === "string" ? cameraKey : activeCameraKey;
        ...
        await waitForCondition(() => playlistReady && pendingEventTime === null && player.readyState >= 2);

        player.pause();
        syncSecondaryPlayers(player.currentTime, false);

        applyViewSelection(normalizedLayout, normalizedCameraKey, { autoplay: false });
        await waitForCondition(() => playlist === allPlaylists[normalizedCameraKey]);

        const targetIndex = findClipIndexBySegmentKey(normalizedCameraKey, normalizedSegmentKey, activeIndex);
        if (targetIndex < 0) {
            throw new Error(`Could not find clip ${normalizedCameraKey}:${normalizedSegmentKey}.`);
        }

        const targetClip = playlist[targetIndex];
    ...
    window.__SENTRYMANAGER_TEST_API = {
